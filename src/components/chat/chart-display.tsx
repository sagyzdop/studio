...
import { suggestChartType } from '@/ai/flows/suggest-chart-type-from-data';
...
export function ChartDisplay({ data, onSimulate }: ChartDisplayProps) {
  ...
  useEffect(() => {
    if (data && data.results) {
      setLoading(true);
      suggestChartType({ data: data.results })
        .then((response) => {
          setChartType(response.chartType);
          toast({
            title: 'Chart Type Suggested',
            description: `AI suggested a ${response.chartType} chart. Reason: ${response.reason}`,
          });
        })
        .catch((error) => {
          ...
          toast({
            variant: 'destructive',
            title: 'AI Error',
            description: 'Could not suggest a chart type.',
          });
          setChartType('table');
        })
        .finally(() => {
